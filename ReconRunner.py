...
# create folder
def createFolder():
    command = (f"mkdir {outputDir} && mkdir {outputDir}/subs && mkdir {outputDir}/subs2 && mkdir {outputDir}/dirs && mkdir {outputDir}/dirs2 && mkdir {outputDir}/portscan && mkdir {outputDir}/portscan2 && mkdir {outputDir}/fuzz")
    try:
...
def findSUBS(url):
    if checkFolderExist() is True:
        print(bcolors.OKGREEN, "[+]", bcolors.OKBLUE, "Folder for saved data exist, no folders will be created!", bcolors.ENDC)
    else:
        print(bcolors.RED, "[-]", bcolors.WARNING, "No folder for saved data exist, creating folders", bcolors.ENDC)
        createFolder()
